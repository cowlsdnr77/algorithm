...
# while len(complete_list) != truck_num:
#     # bridge_q의 요소들 합 + 제일 앞에 남은 트럭 무게 <= 다리가 견딜 수 있는 무게 && bridge_q 길이 < 다리 길이
#     if len(bridge_q) > 0:
#         if len(truck_weights) > 0 and sum(bridge_q) + truck_weights[0] <= weight and len(bridge_q) < bridge_length:
#             bridge_q.append(truck_weights.pop(0))
#             # 여기서 필요한 건 left_time_list에 해당하는 위치를 -1 해야됨
#             left_time_list.append(bridge_length)
#     else:
